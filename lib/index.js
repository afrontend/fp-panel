...
const getMaxRow = fp.pipe(fp.filter(isBlankItem), fp.size);

const firstRowIsBlank = fp.pipe(fp.first, fp.filter(isItem), fp.size, Boolean);

const lastRowIsBlank = fp.pipe(fp.last, fp.filter(isItem), fp.size, Boolean);
...
const getMaxColumn = fp.pipe(fp.map(fp.findLastIndex(isItem)), fp.max);
