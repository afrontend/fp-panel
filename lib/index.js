...
const isOverlap = (aPanel, bPanel) =>
  _.chain()
    .zipWith(_.flattenDepth(aPanel), _.flattenDepth(bPanel))
    .some(isOverlapItem)
    .value();
...
const up = fp.pipe(
  fp.cloneDeep,
  fp.thru(p => [...p.slice(1), getEmptyRow(p[0].length)])
);
...
const down = fp.pipe(
  fp.cloneDeep,
  fp.thru(p => [getEmptyRow(p[0].length), ...p.slice(0, -1)])
);
...
const getTopMargin = panel => {
  const isEmpty =
    !Array.isArray(panel) ||
    panel.length === 0 ||
    _.some(_.first(panel), item => isItem(item));
  return isEmpty ? 0 : 1 + getTopMargin(_.tail(panel));
};
...
const getBottomMargin = panel => {
  const isEmpty =
    !Array.isArray(panel) ||
    panel.length === 0 ||
    _.some(_.last(panel), item => isItem(item));
  return isEmpty ? 0 : 1 + getBottomMargin(_.initial(panel));
};

/**
 * Move some items that has a zeroPoint attribute to the bottom of the panel
 * @param {array} panel - the panel is a 2D array which some items have a color attribute
 * @return {array} new 2D array
 */
